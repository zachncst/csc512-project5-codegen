 ...
 */
public class FunctionCodePrinter {
    public static void print(Node start, PrintWriter pw, VariableHashmap vHMap) {
        printStart(start, pw, vHMap);
        printNode(start, pw, vHMap);
        printEnd(pw);
    }

    public static void printNode(Node start, PrintWriter pw, VariableHashmap vHMap) {
        //leaf node
        if (start.isLeaf()) {
            //print value
        ...
        //data declaration node
            ...
            for (int i = 0; i < start.getChildren().size(); i++) {
                //print children
                printNode(start.getChildren().get(i), pw, vHMap);
            }
        }
    }

    public static void printStart(Node start, PrintWriter pw, VariableHashmap vmap) {
        pw.append("int const N = 2000;\n");
        pw.append("#define top mem[0];\n");
        pw.append("#define base mem[1];\n");
        pw.append("#define jumpReg mem[2];\n");
        pw.append("#define membase 3;\n");
        pw.append("int mem[N];\n");
        pw.append("int main() {\n");
    }
